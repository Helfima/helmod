...
---Get or create sprite tab panel
---@return LuaGuiElement
function UnitTestPanel:getModulesTab()
    return self:getTab("modules-tab-panel", { "helmod_unittest.modules-title" })
end
...
function UnitTestPanel:updateMenu()
    ...
    GuiElement.add(group1,
        GuiButton("HMUnitTestPanel", "reload-script"):sprite("menu", defines.sprites.refresh.black,
            defines.sprites.refresh.black):style("helmod_button_menu"):tooltip("Reload script"))
end
...
function UnitTestPanel:updateEnergy()
    ...
    GuiElement.add(tab_panel, GuiLabel("label"):caption(data.mod):style(defines.styles.label.frame_title))
    ...
    for entity, test_data in spairs(data.energy, function(t, a, b) return t[b]["energy_type_input"] <
        t[a]["energy_type_input"] end) do
...
function UnitTestPanel:addEnergyListRow(itable, entity, test_data)
    ...
    if lua_prototype ~= nil then
        ---col Entity
        local button = GuiElement.add(itable,
            GuiButtonSelectSprite("entity", entity):choose("entity", lua_prototype.name))
        button.locked = true
        ---col Type
...
-------------------------------------------------------------------------------
---Update energy
function UnitTestPanel:updateModules()
    local tab_panel = self:getModulesTab()
    ...
    for _, quality in pairs(qualities) do
        if quality.name ~= "quality-unknown" then
            ...
            for _, lua_module in pairs(modules) do
                if lua_module.subgroup.name ~= "creative-mod_modules" then
                    local module = {type="item", name=lua_module.name, quality=quality.name, amount=1}
                    local module_effects = Player.getModuleEffects(module)
                    ---col Entity
                    local sprite = GuiButtonSelectSprite("item", module.name, quality.name):choose_with_quality("item", module.name,quality.name)
                    table.insert(gui_table.items, sprite)

                    for effect_name, effect_value in pairs(module_effects) do
                        local label_caption = Format.formatPercent(effect_value) .. "%"
                        if effect_value == 0 then
                            label_caption = "-"
                        end
                        local control_value = 0
                        if data.modules[module.name] ~= nil and data.modules[module.name][quality.name] ~= nil and data.modules[module.name][quality.name][effect_name] ~= nil  then
            ...
            local table_panel = GuiElement.add(tab_panel, GuiTable("list-table",quality.name):column(#gui_table.items+1))
            table_panel.vertical_centering = false
            table_panel.style.horizontal_spacing = 10
            table_panel.style.top_margin = 20

            for key, row in pairs(gui_table) do
                if key == "items" then
                    local header = GuiElement.add(table_panel, GuiLabel("label",quality.name):caption(quality.name):style(defines.styles.label.frame_title))
                    header.style.width = 100
                else
                    self:addCellHeaderTooltip(table_panel, key, key)
                end
                for _, cell in pairs(row) do
                    local gui_element = GuiElement.add(table_panel,cell)
                    if key == "items" then
                        gui_element.locked = true
                    end
                end
            end
        end
