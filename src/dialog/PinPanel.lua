...
function PinPanel:onUpdate(event)
    ...
    User.setParameter("show_hidden_pinned_products", nil)
    User.setParameter("show_hidden_pinned_ingredients", nil)
end

local setting_options = {}
table.insert(setting_options,
    { name = "done", icon = defines.sprites.status_ok.white, icon_white = defines.sprites.status_ok.black, tooltip =
    "tooltip.hide-show-done", column = 0 })
table.insert(setting_options,
    { name = "machine", icon = defines.sprites.factory.white, icon_white = defines.sprites.factory.black, tooltip =
    "tooltip.hide-show-factory", column = 1 })
table.insert(setting_options,
    { name = "product", icon = defines.sprites.jewel.white, icon_white = defines.sprites.jewel.black, tooltip =
    "tooltip.hide-show-product", column = 2 })
table.insert(setting_options,
    { name = "beacon", icon = defines.sprites.beacon.white, icon_white = defines.sprites.beacon.black, tooltip =
    "tooltip.hide-show-beacon", column = 1 })
...
function PinPanel:updateHeader(event)
    ...
    for _, setting_option in pairs(setting_options) do
        ...
        if setting_value == true then
            GuiElement.add(group1,
                GuiButton(self.classname, "change-hide", setting_option.name):sprite("menu", setting_option.icon_white,
                    setting_option.icon):style("helmod_button_menu"):tooltip({ setting_option.tooltip }))
        else
            GuiElement.add(group1,
                GuiButton(self.classname, "change-hide", setting_option.name):sprite("menu", setting_option.icon,
                    setting_option.icon):style("helmod_button_menu_selected"):tooltip({ setting_option.tooltip }))
        end
    end

    local group2 = GuiElement.add(action_panel, GuiFlowH("group2"))
    GuiElement.add(group2,
        GuiButton(self.classname, "recipe-done-remove"):sprite("menu", defines.sprites.checkmark.black,
            defines.sprites.checkmark.black):style("helmod_button_menu_actived_red"):tooltip({
            "helmod_button.remove-done" }))
    ...
    GuiElement.add(group3,
        GuiButton("HMSummaryPanel", "OPEN", model.id, block.id):sprite("menu", defines.sprites.list_view.black,
            defines.sprites.list_view.black):style("helmod_button_menu"):tooltip({
            "helmod_result-panel.tab-button-summary" }))

    local tooltip = GuiTooltipBuilding("tooltip.smart-pipette"):element(block):byLimit(block.by_limit)
    GuiElement.add(group3,
        GuiButton(self.classname, "building-constant", model.id, block.id):sprite("menu", defines.sprites.factory.black,
            defines.sprites.factory.black):style("helmod_button_menu"):tooltip(tooltip))

    local group4 = GuiElement.add(action_panel, GuiFlowH("group4"))
    GuiElement.add(group4,
        GuiButton("HMProductionPanel", "OPEN", model.id, block.id):sprite("menu", defines.sprites.hangar.black,
            defines.sprites.hangar.black):style("helmod_button_menu"):tooltip({
            "helmod_result-panel.tab-button-production-block" }))
end
...
function PinPanel:addProductionBlockHeader(itable)
    ...
    if not (User.getSetting("pin_panel_column_hide_product")) then
        ...
        GuiElement.add(guiIngredients,
            GuiLabel("header-ingredients"):caption({ "helmod_result-panel.col-header-ingredients" }))
    end
...
function PinPanel:addProductionBlockRow(gui_table, model, block, recipe)
    ...
    if is_done == true then
        GuiElement.add(gui_table, GuiButton(self.classname, "recipe-done", recipe.id):sprite("menu", defines.sprites.status_ok.black, defines.sprites.status_ok.black)
            :style("helmod_button_menu_selected_green"):tooltip({"helmod_button.done" }))
    else
        GuiElement.add(gui_table, GuiButton(self.classname, "recipe-done", recipe.id):sprite("menu", defines.sprites.checkmark.black, defines.sprites.checkmark.black)
            :style("helmod_button_menu_actived_green"):tooltip({ "helmod_button.done" }))
    end
    ---col recipe
    local cell_recipe = GuiElement.add(gui_table, GuiFrameH("recipe", recipe.id):style(helmod_frame_style.hidden))
    local button_recipe = GuiCellRecipe("HMRecipeEdition", "OPEN", model.id, block.id, recipe.id):element(recipe)
        :infoIcon(recipe.type):tooltip("tooltip.edit-recipe"):color(GuiElement.color_button_default):mask(is_done)
    ...
    if not (User.getSetting("pin_panel_column_hide_product")) then
        ...
        if lua_products ~= nil then
			...
			local show_hidden_pinned_products = User.getParameter("show_hidden_pinned_products")
            local skip_hidden_products = show_hidden_pinned_products == recipe.id
            ...
            for index, lua_product in pairs(lua_products) do
                ...
                if skip_hidden_products or display_hidden_products == 0 or product.count > display_hidden_products then
                    GuiElement.add(cell_products, GuiCellElementSm(self.classname, "pipette-item", recipe.id, "product", index):index(index):element(product)
                        :tooltip("tooltip.smart-pipette"):color(GuiElement.color_button_none):byLimit(block.by_limit):mask(is_done):controlInfo("pipette-item"):withLogistic())
                else
            ...
            if #hidden_products > 0 then
				...
				GuiElement.add(cell_products, GuiCellHiddenElementSm(self.classname, "show-hidden-pinned-products", recipe.id, "product"):element(hidden_products):tooltip("tooltip.add-recipe")
					:color(product_color):withLogistic())
			end
        end
    end
    ...
    if not (User.getSetting("pin_panel_column_hide_machine")) then
        ...
        if factory ~= nil then
            GuiElement.add(cell_factory,
                GuiCellFactory(self.classname, "pipette-entity", recipe.id, "factory"):index(recipe.id):element(factory)
                :tooltip("tooltip.smart-pipette"):color(GuiElement.color_button_default):byLimit(block.by_limit):mask(
                is_done))
        end
    end
    ...
    if not (User.getSetting("pin_panel_column_hide_product")) then
        ...
        if lua_ingredients ~= nil then
            local hidden_products = {}
			local show_hidden_pinned_ingredients = User.getParameter("show_hidden_pinned_ingredients")
            local skip_hidden_products = show_hidden_pinned_ingredients == recipe.id
            ...
            for index, lua_ingredient in pairs(lua_ingredients) do
                ...
                if skip_hidden_products or display_hidden_products == 0 or ingredient.count > display_hidden_products then
                    GuiElement.add(cell_ingredients, GuiCellElementSm(self.classname, "pipette-item", recipe.id, "ingredient", index):index(index):element(ingredient)
                        :tooltip("tooltip.smart-pipette"):color(GuiElement.color_button_add):byLimit(block.by_limit):mask(is_done):controlInfo("pipette-item"):withLogistic())
                else
            ...
            if #hidden_products > 0 then
				...
				GuiElement.add(cell_ingredients, GuiCellHiddenElementSm(self.classname, "show-hidden-pinned-ingredients", recipe.id, "ingredient")
                    :element(hidden_products):tooltip("tooltip.add-recipe"):color(product_color):withLogistic())
			end
        end
    end
    ...
    if not (User.getSetting("pin_panel_column_hide_beacon")) then
        ...
        if beacons ~= nil then
            for index, beacon in pairs(beacons) do
                ...
                GuiElement.add(cell_beacons,
                    GuiCellFactory(self.classname, "pipette-entity", recipe.id, "beacons", index):index(index):element(
                    beacon):tooltip("tooltip.smart-pipette"):color(GuiElement.color_button_default):byLimit(block
                    .by_limit):mask(is_done))
            end
        end
    end
end
...
function PinPanel:onEvent(event)
    if event.action == "show-hidden-pinned-products" then
		User.setParameter("show_hidden_pinned_products", event.item1)
		Controller:send("on_gui_update", event, self.classname)
	end
    
    if event.action == "show-hidden-pinned-ingredients" then
		User.setParameter("show_hidden_pinned_ingredients", event.item1)
		Controller:send("on_gui_update", event, self.classname)
	end
