...
-------------------------------------------------------------------------------
---Build a constant combinator from a list of inputs and place it on the cursor.
---Counts come from the user's combinator preferences (item quantity mode/value
---and flat fluid amount); negative values produce a request list.
---@param items table list of {type, name, quality, amount}
---@return any
function Player.setSmartToolInputsConstantCombinator(items)
    if Lua_player == nil or items == nil or #items == 0 then
        return nil
    end
    local mode = User.getPreferenceSetting("combinator_item_quantity_mode")
    local item_value = User.getPreferenceSetting("combinator_default_item_value")
    local fluid_value = User.getPreferenceSetting("combinator_default_fluid_amount")
    ...
    for _, item in ipairs(items) do
        local count
        if (item.type or "item") == "fluid" then
            count = round(fluid_value)
        elseif mode == "amount" then
            count = round(item_value * (item.amount or 0))
        else
            local stack_size = ItemPrototype(item.name):stackSize()
            count = round(item_value * (stack_size or 0))
        end
        if count ~= 0 then
            local filter = {
                index = signal_index,
