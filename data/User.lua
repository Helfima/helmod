...
function User.isReader(model)
    return Player.isAdmin() or model.owner == Player.native().name or
    (model.share ~= nil and bit32.band(model.share, 1) > 0)
end
...
function User.isWriter(model)
    return Player.isAdmin() or model.owner == Player.native().name or
    (model.share ~= nil and bit32.band(model.share, 2) > 0)
end
...
function User.isDeleter(model)
    return Player.isAdmin() or model.owner == Player.native().name or
    (model.share ~= nil and bit32.band(model.share, 4) > 0)
end
...
function User.isFilterTranslate()
    return User.getModGlobalSetting("filter_translated_string_active") and
    (User.getParameter("filter-language") == nil or User.getParameter("filter-language") == "left")
end
...
function User.setParameterObjects(classname, model_id, block_id, recipe_id)
    ...
    User.setParameter(parameter_objects, { name = parameter_objects, model = model_id, block = block_id, recipe =
    recipe_id })
end
...
function User.getThumbnailColor(parameter)
    local thumbnails_color = User.getThumbnailsColor()
    local thumbnail_color = thumbnails_color[parameter]
    return thumbnail_color
end

-------------------------------------------------------------------------------
---Get Thumbnails Color
---@return ThumbnailsColorData
